...
func main() {
	...
	rootCmd := &cobra.Command{
		Use:               "epsd",
		Short:             "EPS blockchain Daemon (server)",
	...
	// CLI command to print the epsd version
	rootCmd.AddCommand(version.Cmd())
	// CLI command to start and run the epsd (server)
	server.AddCommands(ctx, cdc, rootCmd, newApp, exportAppStateAndTMValidators)

	// prepare and add flags
	executor := cli.PrepareBaseCmd(rootCmd, "EPS", app.DefaultNodeHome)
...
func exportAppStateAndTMValidators(
	...
	if height != -1 {
		gApp := app.NewEpsApp(logger, db, traceStore, false, uint(1))
	...
	gApp := app.NewEpsApp(logger, db, traceStore, true, uint(1))
	return gApp.ExportAppStateAndValidators(forZeroHeight, jailWhiteList)
}
